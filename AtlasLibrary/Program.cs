...
{
    client.BaseAddress = new Uri(builder.Configuration["ApiSettings:LoansApiAdress"]!);
});
...
// HttpClient för items/object API
builder.Services.AddHttpClient("EquipmentItemsApi", client =>
{
    client.BaseAddress = new Uri(builder.Configuration["ApiSettings:EquipmentItemsApiBaseUrl"]!);
});
...
builder.Services.AddHttpClient<ItemsService>((serviceProvider, httpClient) =>
{
    ...
    string? adress = config.GetValue<string>("ApiSettings:ItemsServiceAdress");

    if (string.IsNullOrWhiteSpace(adress))
    {
        throw new InvalidOperationException("ItemsServiceAdress saknas i appsettings.");
    }
